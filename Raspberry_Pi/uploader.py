...
class MotionUploader:
    ...
    def upload_video(self, video_file_path):
        ...
        if self.send_email:
			...
			link_pass = video_link
                        #Command='"DeviceId=dv1&Link='+video_link+'"'
			#print Command
			#os.system("curl --data ${Command} --url http://sssdo.host56.com/phpFiles/loadGcmIdByDeviceId.php")
			link_pass = '"' + link_pass + '"' 
...
if __name__ == '__main__':         
    try:
        if len(sys.argv) < 3:
            exit('Motion Uploader - Upload Videos to Google Drive\n')
	    print link_pass
        cfg_path = sys.argv[1]
        vid_path = sys.argv[2]    
